...
class BaseBoilerplate extends Command {
  ...
  async run() {
    // ask user for input
    this.locals = Object.assign(this.locals, await this.initLocals());
    this.questions = this.initQuestions();
    await this.askQuestions();

    debug('locals: %j', this.locals);
  ...
  async askQuestions() {
    ...
    const questions = this.questions.filter(q => {
      if (this.context.argv[q.name] === undefined) return true;
      debug('skip question: %s', q.name);
      return false;
    });
  ...
  async normalizeFileInfo({ fileInfo }) {
    ...
    fileInfo.dest = path.join(this.context.argv.baseDir, fileName);
  }
  ...
  async saveFile({ fileInfo }) {
    ...
    debug('save file to: %j', fileInfo.dest);
  }
