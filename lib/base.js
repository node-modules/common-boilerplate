...
class BaseBoilerplate extends Command {
  ...
  async run() {
    ...
    this.logger.success('finish job, enjoy yourself.');
  }
   ...
   * @param {Object} [defaults] - default values for question
   * @param {Object} [opts] - some opts for question create
   * @return {Object} question definition
   */
  getBuiltinQuestions(key, defaults = {}, opts = {}) {
    ...
    switch (key) {
      ...
      case 'name': {
        ...
        return {
          ...
          filter(v, obj) {
            ...
            obj.pkgName = obj.scope ? `${obj.scope}/${v}` : v;
            return v;
          },
          transformer(v) {
      ...
      case 'repository': {
        return {
          ...
          filter(v, obj) {
            const gitInfo = helper.hostedGitInfo.fromUrl(v);
            if (gitInfo) {
              obj.homepage = gitInfo.browse();
              obj.gitInfo = helper.parseGitUrl(gitInfo.ssh());
            }
            return v;
          },
        };
